...
class Exp_Long_Term_Forecast_withPCA(Exp_Basic):
    def __init__(self, args):
        super(Exp_Long_Term_Forecast_withPCA, self).__init__(args)
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        if self.args.output_attention:
                            outputs = self.model.predict_and_retrieve(batch_x, batch_x_mark, dec_inp, batch_y_mark)[0]
                        else:
                            outputs = self.model.predict_and_retrieve(batch_x, batch_x_mark, dec_inp, batch_y_mark)
                    ...
                    f_dim = -1 if self.args.features == 'MS' else 0
                    outputs = outputs[:, -self.args.pred_len-self.args.label_len:, f_dim:]
