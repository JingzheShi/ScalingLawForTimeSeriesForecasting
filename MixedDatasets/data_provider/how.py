...
class Dataset_Custom(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 ...
                 target='OT', scale=True, timeenc=0, freq='h',
                 percent=100, max_len=-1, train_all=False):
        print("Now we use randomly percent")
        # size [seq_len, label_len, pred_len]
        # info
        if size == None:
            self.seq_len = 24 * 4 * 4
        ...
        if flag == 'train':
            ...
            print("Now flag is {}, so we use percent=100.".format(flag))
            self.percent = 100
        ...
        self.enc_in = self.data_x.shape[-1]
        self.tot_len = self.length//self.enc_in # len(self.data_x) - self.seq_len - self.pred_len + 1
    ...
    def __read_data__(self):
        ...
        self.data_stamp = data_stamp
        self.enc_in = self.data_x.shape[-1]
        print("enc_in ==", self.enc_in)
        self.length = (len(self.data_x) - self.seq_len - self.pred_len + 1) * self.enc_in
        
        if self.percent == 100:
            print("Now using self.percent==100.")
        ...
        if self.set_type == 0:
            border2 = (border2 - self.seq_len) * 100 // 100 + self.seq_len
        ...
        each_length = self.seq_len + self.pred_len
        
        while not success_flag:
            periods_num = (total_length // N) - 1
            if periods_num < 30:
                success_flag = False
                N = int(N/1.2)
                continue
            success_flag = True
            print("Current N = ", N)
            period_indices = torch.randperm(periods_num).tolist()
            period_indices = period_indices[:int(periods_num * self.percent / 100)]
            print("Current period_indices = ", period_indices)
            time_indices = []
            zero_through_N_minus_1 = torch.arange(N - each_length + 1)
            for i in period_indices:
                time_indices.extend((zero_through_N_minus_1 + i * N).tolist())
                
            
            self.time_indices = time_indices
            self.length = len(time_indices) * self.enc_in
            print("self.length ==", self.length)
        # N /= 2
        
        # print("self.data_x.shape,", self.data_x.shape)
        # print("self.data_y.shape",self.data_y.shape)
        # print("self.data_stamp.shape",self.data_stamp.shape)
    ...
    def __getitem__(self, index):
        feat_id = index // self.tot_len
        if self.percent != 100:
            s_begin = self.time_indices[index % self.tot_len] # only several time period is visible
        else:
            s_begin = index % self.tot_len
