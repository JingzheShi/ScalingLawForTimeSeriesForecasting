...
class Gated_MLP(nn.Module):
    def __init__(self,in_dim,out_dim,hid_dim=None,activation='gelu',dropout=0.0):
        super().__init__()
        ...
        self.drop_out = nn.Identity() # if dropout == 0 else nn.Dropout(dropout)
    def forward(self,x):
...
class ResMLP(nn.Module):
    def __init__(self, in_and_out_dim, activation, layer_num, hidden_dim=None,dropout=0.0):
        ...
        for _ in range(layer_num):
            self.mlp_layers.append(Gated_MLP(in_and_out_dim, in_and_out_dim, hidden_dim, activation,dropout=dropout))
