...
class ResMLP(nn.Module):
    def __init__(self, in_and_out_dim, activation, layer_num, hidden_dim=None):
        ...
        WIDTH=2
        print("WIDTH", WIDTH)
        hidden_dim = int(WIDTH*in_and_out_dim) if hidden_dim is None else hidden_dim
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        projector_list = []
        # projector_list.append(ResMLP(configs.d_model, configs.activation, num_layers))
        ...
        past_projector_list = []
        # past_projector_list.append(ResMLP(configs.d_model, configs.activation, num_layers))
        ...
        mask_encoder_list = []
        mask_encoder_list.append(MLP(configs.seq_len, configs.d_model, activation = configs.activation))
        # mask_encoder_list.append(ResMLP(configs.d_model, configs.activation, num_layers-1))
        self.mask_encoder = nn.Sequential(*mask_encoder_list)
